...
public class SlidingWindowDiversityOperationRuleTest {
    ...
    public void scattersCategoryWithinEveryWindow() {
        ...
        context.addData("operationItemMap", items(item("a1", "a", "x"), item("a2", "a", "y"), item("a3", "a", "z"),
    ...
    public void appliesEveryTagAndCompositeDimensions() {
        ...
        assertEquals(2, SlidingWindowDiversityOperationRule.keys(item, Collections.singletonList("tags")).size());
        assertTrue(SlidingWindowDiversityOperationRule.keys(item, Collections.singletonList("category+tags"))
            .contains("category+tags=sports+nba"));

        GraphContext context = context(3, 3, 1, "category+tags");
        List<ScoreResult> input = scores("a", "b", "c");
        context.addData("operationItemMap",
            items(item("a", "sports", "nba"), item("b", "sports", "cba"), item("c", "sports", "nba")));
        assertEquals(Arrays.asList("a", "b"), ids(new SlidingWindowDiversityOperationRule().handle(context, input)));
    }
    ...
    public void returnsShorterResultWhenConstraintCannotBeSatisfied() {
        ...
        List<ScoreResult> input = scores("a", "b", "c");
        context.addData("operationItemMap",
            items(item("a", "same", "x"), item("b", "same", "y"), item("c", "same", "z")));
    ...
    public void doesNotApplyChannelRatios() {
        ...
        input.get(0).setRecallFrom("item_cf_i2i");
        input.get(1).setRecallFrom("item_cf_i2i");
        input.get(2).setRecallFrom("hot");
        input.get(3).setRecallFrom("hot");
        context.addData("operationItemMap",
            items(item("i2i-1", "a", "x"), item("i2i-2", "b", "y"), item("hot-1", "c", "z"), item("hot-2", "d", "w")));
    ...
    private static GraphContext context(int size, int window, int repeat, String... dimensions) {
        ...
        operation.setWindowSize(window);
        operation.setRepeatK(repeat);
    ...
    private static List<ScoreResult> scores(String... ids) {
        ...
        for (int i = 0; i < ids.length; i++) {
            result.add(new ScoreResult(ids[i], ids.length - i));
        }
        return result;
    }
    ...
    private static void assertWindowLimit(List<ScoreResult> result, Map<String, Item> items, int window, int repeat,
        List<String> dimensions) {
        for (int start = 0; start + window <= result.size(); start++) {
            Map<String, Integer> counts = new LinkedHashMap<>();
            for (int i = start; i < start + window; i++) {
                for (String key : SlidingWindowDiversityOperationRule.keys(items.get(result.get(i).getId()),
                    dimensions)) {
