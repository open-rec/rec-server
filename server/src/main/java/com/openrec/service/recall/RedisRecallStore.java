...
public class RedisRecallStore implements RecallStore {
    ...
    private static final String I2I_KEY = "i2i:{%s}:%s";
    ...
    public List<ScoreResult> hot(String scene, int size) {
        return redisService.getZSet(String.format(HOT_KEY, scene), 0, Double.MAX_VALUE, size);
    }
    ...
    public List<ScoreResult> newest(String scene, long startTime, long endTime, int size) {
        List<ScoreResult> result = redisService.getZSet(
            String.format(NEW_KEY, scene), startTime, endTime, size);
        if (endTime > 0) {
    ...
    public List<ScoreResult> i2i(String scene, List<String> triggerItems, int size) {
        ...
        for (String trigger : triggerItems) {
            keys.add(String.format(I2I_KEY, trigger, scene));
        }
