...
public class KafkaServiceUnitTest {
    @Test public void serializesEachDomainObjectToItsTopic() {
        ...
        Item item = new Item(); item.setId("i"); User user = new User(); user.setId("u"); Event event = new Event(); event.setItemId("i");
        service.writeItem(item); service.writeUser(user); service.writeEvent(event);
        verify(template).send(eq("items"), contains("\"id\":\"i\""));
        verify(template).send(eq("users"), contains("\"id\":\"u\""));
        verify(template).send(eq("events"), contains("\"itemId\":\"i\""));
    }
}
